# Dynamic Config Management using Spring Cloud Config
...
There can be multiple clients talking to server over http. It the clients are spring boot, there are other mechanisms such as client refresh to which will update the config data from the server. Every client is expected to be aware of the Spring cloud config server.
